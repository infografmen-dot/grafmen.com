...
for (const filename of readdirSync(join(root, 'content/portfolio')).filter(f => f.endsWith('.json'))) {
  ...
  const fields = {
    TITLE: escape(data.title), DESCRIPTION: escape(data.description),
    ...
    CATEGORY: escape(data.category), YEAR: data.year ? ' / ' + escape(data.year) : '',
    COVER: imagePath(data.cover), COVER_ALT: escape(data.cover_alt), BRIEF_TITLE: escape(data.brief_title),
    BRIEF: data.brief.map(t => `<p>${escape(t)}</p>`).join(''),
    DELIVERABLES: data.deliverables.map(t => `<li>${escape(t)}</li>`).join(''),
    GALLERY: data.gallery.map(i => `<figure><img src="${imagePath(i.src)}" alt="${escape(i.alt)}" loading="lazy" height="${i.height}" width="${i.width}"><figcaption>${escape(i.caption)}</figcaption></figure>`).join(''),
    RESULT_TITLE: escape(data.result_title), RESULT: data.result.map(t => `<p>${escape(t)}</p>`).join(''),
    TESTIMONIAL: data.testimonial && data.testimonial_author ? `<blockquote><p>${escape(data.testimonial)}</p><cite>${escape(data.testimonial_author)}</cite></blockquote>` : '',
    EXTERNAL: data.external_url ? `<a class="text-link" href="${link(data.external_url)}">${escape(data.external_label)} ↗</a>` : '',
    NEXT_URL: link(data.next_url), NEXT_TITLE: escape(data.next_title)
  };
